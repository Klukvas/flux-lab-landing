...
import { getPositionById, getPositionIds } from "@/data/careers";
...
export async function generateStaticParams() {
  return locales.flatMap((locale) =>
    getPositionIds().map((id) => ({ locale, id })),
  );
}
  ...
  return (
    <>
      <JobPostingJsonLd
        position={position}
        locale={locale}
        labels={{ requirements: t("requirements"), benefits: t("benefits") }}
      />
      <BreadcrumbJsonLd
        locale={locale}
        items={[
          { name: tNav("home"), path: "" },
          { name: t("title"), path: "/careers" },
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24">
                <Card>
                  ...
                  <ApplicationForm defaultPosition={position.title} />
                </Card>
              </div>
            </div>
          </div>
