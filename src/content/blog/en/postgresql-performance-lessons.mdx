...
description: PostgreSQL optimization lessons from running production SaaS databases at fluxLab.dev: indexing, query tuning, N+1 queries and connection pooling.
date: 2025-02-05
author: fluxLab.dev
...
We tried soft deletes (adding a `deleted_at` column) in our [Accounting](/en/projects/accounting) product. Every query needed `WHERE deleted_at IS NULL`, and we missed it in several places. Now we prefer hard deletes with proper foreign key constraints and a separate audit log table when we need history.
...
If your database is slowing your product down, a performance audit is part of our [technical consulting](/en/services).
