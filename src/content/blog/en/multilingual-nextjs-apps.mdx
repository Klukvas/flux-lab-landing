...
This tells Google which version to show to Ukrainian vs. English speakers. The `x-default` entry names the page for everyone else; we point it at the English version.
...
Our sitemap generates URLs for every page × every locale combination. For our portfolio site with 6 projects, 11 blog posts and our job openings, that's about 60 sitemap entries, each listing its language alternates.
...
Building a product for more than one market? We ship multilingual Next.js apps for clients too: [see our services](/en/services).
